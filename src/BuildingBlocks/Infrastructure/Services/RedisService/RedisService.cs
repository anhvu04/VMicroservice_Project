...
namespace Infrastructure.Services.RedisService;
...
public class RedisService(IConnectionMultiplexer redis) : ICacheService
{
    ...
    {
        var result = await _database.StringSetAsync(key, JsonSerializer.Serialize(value, _jsonSerializerOptions), expiry);
        return result;
    }
